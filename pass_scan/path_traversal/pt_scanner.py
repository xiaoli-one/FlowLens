# 目录遍历 / 任意文件读取（Path Traversal / Arbitrary File Read）检测流程。
#
# 这个文件负责“怎么检测目录遍历/任意文件读取”，自带一份精简的主动发包逻辑
# （不复用 SQL / XSS / RCE 插件，各插件互不影响）。
#
# 判定思路（低误报的关键，与命令注入的算术回显同理）：
#   注入遍历 payload 去读系统里“人人都有、内容特征唯一”的文件，只有目标真的把文件
#   内容读出来并回显，响应里才会出现该文件的 signature（如 /etc/passwd 的 root:x:0:0:、
#   win.ini 的 [fonts]）。参数被原样反射只会回显 payload 字符串（../../etc/passwd），
#   不含 signature，因此能天然区分“真读到文件”和“普通反射”，误报极低。
#   再要求 signature 不在 baseline 里，进一步排除页面本身就含该特征的极端情况。
#
# 两种检测方式（报告里统一归为一种漏洞“任意文件读取”，用证据里的“检测方式”区分）：
#   1. 文件内容回显（technique=content）：读 /etc/passwd（unix）/ win.ini（windows），
#      响应命中对应 signature 即确认。
#   2. PHP 源码读取（technique=php_source）：php://filter 把脚本 base64 后回显，
#      解码出现 <?php / <?= 即确认（读到了服务端源码，属任意文件读取）。
#
# 流量筛选（先筛后测，满足“需要才进入检测”）：
# - 全局层：ScanContext 已过滤静态资源、二进制响应、无意义方法/状态码。
# - 插件层 interested()：GET/POST + 有可控参数才进入。默认对全部参数做目录遍历/文件读取
#   检测，不做参数名/值筛选（full_payload_scan 时再额外纳入 Cookie/Header）。
#
# OS 家族记忆：某 host 一旦通过某类命中确认是 unix / windows，后续按该 OS 裁剪 payload
# （Linux 不再发 win.ini），减少无效请求，参考命令注入的 host_os。
#
# tamper 阶段：主流绕过编码已并进基础 payload；只有“未编码” payload 被 WAF 拦、或开启
# deep_tamper 且未命中时，才按 rules.yaml 的 tamper_profiles 追发少量编码变形兜底。
...
from pass_scan.path_traversal.rules import (
    build_probe_groups,
    max_tampered_payloads_per_probe,
    tamper_profiles,
    waf_detection_rules,
)
from pass_scan.path_traversal.tamper import apply_tampers
...
# PHP 源码读取的命中哨兵（rules.yaml 里 php_b64 的 signature）。
...
class PathTraversalScanner:
    ...
    接口和 SQLInjectionScanner / CommandInjectionScanner / XSSScanner 对齐：
    - name
    - interested(context)
    - dedup_key(context)
    - check(context)
    ...
    def __init__(self, config=None, vuln_file=None, report_file=None):
        ...
        self.deep_tamper_max_profiles = int(self.config.get("deep_tamper_max_profiles", 3))
    ...
    def extract_candidates(self, context):
        ...
        默认对全部参数做目录遍历/文件读取检测，不做“疑似文件/路径参数”筛选：
    ...
    def check_one_param(self, flow, baseline, candidate):
        ...
        for group in groups:
            ...
            # php://filter 需要参数原本就像脚本资源，避免对纯数字/无意义参数空发。
            if group["technique"] == "php_source" and not self.php_worth_trying(candidate):
                continue
            ...
            for probe in self.send_probe_variants(
                ...
                hit = self.match_signature(group, probe, baseline_text)
                if not hit:
                    continue
                ...
                return self.build_finding(
                    flow, candidate,
                    {
                        ...
                        "message": self.hit_message(group["technique"]),
                        "os": group["os"],
                        "target_file": group.get("target"),
    ...
    def match_signature(self, group, probe, baseline_text):
    ...
    def match_php_base64(self, text):
        """在响应里找 base64 大块，解码后判定是否读到了敏感内容。

        php://filter 既能读脚本源码（解码后见 <?php / <?=），也能读任意文件
        （如把 /etc/passwd base64 出来，解码后见 root:x:0:0:）。两种都算任意文件读取。
        """
        for block in _BASE64_BLOCK_RE.findall(text)[:20]:
            # base64 长度需为 4 的倍数才好解码，截到最近的 4 倍数。
            trimmed = block[: len(block) - (len(block) % 4)]
            if len(trimmed) < 40:
                continue
            try:
                # binascii.Error 是 ValueError 的子类，捕获 ValueError 即可。
                decoded = base64.b64decode(trimmed, validate=True)
            except ValueError:
                continue
            head = decoded[:4000].decode("utf-8", errors="replace")
            for label, pattern in _PHP_DECODED_SIGNATURES:
    ...
    def php_worth_trying(self, candidate):
        """php://filter 只在参数值像脚本资源（空、含 . 或看起来是文件名）时才发。"""
    ...
    def technique_label(self, technique):
        return {"content": "文件内容回显", "php_source": "PHP 源码读取"}.get(
            technique, "文件内容回显"
        )

    def hit_message(self, technique):
        if technique == "php_source":
            return "server returned base64-encoded PHP source via php://filter"
        return "sensitive file content signature appeared in response"
    ...
    def urlencode_pairs(self, pairs, charset="utf-8"):
        """编码 query/form 参数，保留 payload 里手写的 %xx（如 %2f、%00）。"""
        encoded = []
    ...
    def build_finding(self, flow, candidate, evidence, proof_probe=None, extra_probes=None):
        ...
        两种检测方式（文件内容回显 / PHP 源码读取）统一为一种漏洞：
        type=method=path_traversal，报告里显示为“任意文件读取”，
        具体是哪种方式看 evidence["technique"]。
        """
        request = flow.request
        finding = {
