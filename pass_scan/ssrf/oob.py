# SSRF OOB（带外/Out-of-Band）关联管理器。
#
# SSRF 的核心确认信号是“服务端真的按我给的地址发起了一次外部请求”。带外通道是判定它最
# 通用、最可靠的方式：把参数替换成指向唯一子域的 URL（http://<host>/），服务端一旦去解析
# 或拉取这个地址，就会回连我们控制的 OOB 服务器；轮询到带对应 token 的交互，就能关联回
# 具体注入点，判定 SSRF 成立。回连来源 IP 是目标服务器而非我们，天然证明是“服务端”发起。
# OOB 命中即高置信，且完全不依赖响应是否回显，覆盖“盲打 SSRF”（绝大多数真实场景）。
#
# 这里对接 projectdiscovery 的免费公共 interactsh 服务（oast.pro 等），协议实现在
# tools/interactsh_client.py（各插件共享的底层协议客户端）。本模块只负责：注册会话、给注入点
# 发放唯一域名、后台轮询、把回连关联回注入点并回调上层生成漏洞结果。与命令注入插件各自持有
# 独立会话，互不影响。
#
# 前提：目标必须能对外发起请求（DNS/HTTP 出站）。内网无出网环境 OOB 会静默；当前 SSRF
# 插件只做 OOB 确认，不把响应错误或回显单独作为 SSRF 漏洞结论。
...
class OOBManager:
    ...
    def start(self):
        ...
        if not self.config.get("enabled", False):
            return False
        worker = threading.Thread(target=self._bootstrap, name="pass-scan-ssrf-oob-init", daemon=True)
        worker.start()
        return True
    ...
    def _bootstrap(self):
        ...
        try:
            ...
            print(yellow(f"[SSRF-OOB] 加载 interactsh 客户端失败，OOB 不可用: {error}"), flush=True)
            return
        ...
        for server in servers:
            ...
            if client.register():
                self.client = client
                self.enabled = True
                print(yellow(f"[SSRF-OOB] 已连接 interactsh 服务: {server}"), flush=True)
                self._poll_loop()
                return

        print(
            yellow("[SSRF-OOB] 所有 interactsh 服务器注册失败，OOB 不可用（将只用响应回显检测）"),
            flush=True,
        )
        ...
        for interaction in interactions:
            ...
            if self.on_interaction:
                try:
                    ...
                    print(yellow(f"[SSRF-OOB] 处理回连异常: {error}"), flush=True)
        return hits
